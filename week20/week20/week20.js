...
function makeTwo() {
  ...
  try {
    ...
    resultTwo.textContent = `Ошибка была перехвачена: ${err.message}`;
  }
}
...
function makeFour() {
  ...
  try {
    makeNull();
    resultFour.textContent = "Функция выполнена";
  } catch (err) {
    resultFour.textContent = `Результат умножения: ${2 * 8}`;
  }
}
...
function makeFive() {
  ...
  if (inputData < 0) {
    throw new Error("input data less than 0");
  }
...
function makeTen() {
  ...
  try {
    ...
    resultTen.textContent = `Ошибка перехвачена: ${err.message}`;
  }
}
...
function makeTwelve() {
  let age = 12;
  if (age < 18) {
    throw new Error("Доступ запрещен лицам младше 18 лет");
  } else {
...
function makeFourteen() {
  ...
  try {
    ...
    console.log("Замените текст: " + err.message);
  }
}
...
function makeSixteen(n, func) {
  ...
  try {
    ...
    resultSixteen.textContent = err.message;
  }
}
...
function makeSeventeen(str, func) {
  ...
  try {
    ...
    resultSeventeen.textContent = err.message;
  }
}
...
function makeTwenty() {
  ...
  const result20a = document.getElementById("result20a");
  try {
    const value = calculateValue();
    result20.textContent = value;
  } catch (error) {
    result20.textContent = "Ошибка";
  } finally {
    result20a.textContent = "finally20";
  }
}
...
function makeTwentyOne() {
  ...
  try {
    ...
    result21.textContent = data;
  } catch (error) {
    result21.textContent = "Ошибка";
  } finally {
...
function makeTwentySix() {
  ...
  setTimeout(() => {
    try {
      ...
      resultTwentySix.textContent = "Произошла ошибка: " + error.message;
