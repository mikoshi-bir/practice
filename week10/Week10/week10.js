...
function secondGreet(guestName = 'гость') {
	return 'Привет, ' + guestName;
}
console.log(secondGreet('Kate'));
...
console.log(
	'С момента летней олимпиады в Токио прошло ' + calculateYearsSinceLastOlympics(year, lastOlympicsYear) + ' года',
);
...
function calculateAge(friendYearOfBirth) {
	...
	const age = currentYear - friendYearOfBirth;
  	console.log(`Моему другу сейчас ${age} лет`);
}
calculateAge(1985);
...
function calculateMyAge(yearOfBirth) {
	const currentYear = new Date().getFullYear();
  	...
  	console.log(`Мне сейчас ${age} лет`);
}
calculateMyAge(2000);
